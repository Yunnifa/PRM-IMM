...
const DataUser = () => {
  ...
  const [formData, setFormData] = useState({ 
    ...
    birthDate: '',
    department: '', 
    role: 'user' as 'admin' | 'head_dept' | 'ga' | 'user'
  });
  ...
  const handleEdit = (user: User) => {
    ...
    setFormData({ 
      ...
      role: user.role as 'admin' | 'head_dept' | 'ga' | 'user'
    });
  ...
  const exportToCSV = () => {
    ...
    const headers = ['ID', 'Username', 'Nama Lengkap', 'WhatsApp', 'Email', 'Department', 'Role'];
    const csvData = filteredUsers.map(u => [u.id, u.username, u.fullName, u.whatsapp || '-', u.email, u.department || '-', u.role]);
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text('Data User', 14, 15);
    doc.setFontSize(10);
    doc.text(`Tanggal: ${new Date().toLocaleDateString('id-ID')}`, 14, 22);
    
    autoTable(doc, {
      startY: 30,
      head: [['ID', 'Username', 'Nama Lengkap', 'WhatsApp', 'Email', 'Department', 'Role']],
      body: filteredUsers.map(u => [u.id, u.username, u.fullName, u.whatsapp || '-', u.email, u.department || '-', u.role]),
      styles: { fontSize: 8 },
    ...
    reader.onload = (e) => {
      try {
        ...
        jsonData.forEach((row, index) => {
          ...
          const newId = users.length + importedUsers.length + 1;
    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="max-h-[60vh] overflow-y-auto overflow-x-auto">
          <table className="w-full min-w-[900px]">
            ...
            <tr>
              ...
              <th className="px-2 lg:px-4 py-2 lg:py-3 text-left text-xs lg:text-sm font-semibold">Role</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {paginatedUsers.map((user, index) => (
              <tr key={user.id} className="hover:bg-gray-50">
                ...
                <td className="px-2 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-gray-700">{user.username}</td>
                <td className="px-2 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-gray-900">{user.fullName}</td>
                ...
                <td className="px-2 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-gray-700">{user.email}</td>
                <td className="px-2 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm text-gray-700">{user.department || '-'}</td>
                <td className="px-2 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm">
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                    user.role === 'admin' ? 'bg-purple-100 text-purple-800' :
                    user.role === 'head_dept' ? 'bg-blue-100 text-blue-800' :
                    user.role === 'ga' ? 'bg-green-100 text-green-800' :
                    'bg-gray-100 text-gray-800'
                  }`}>
                    {user.role === 'head_dept' ? 'Head Department' : user.role === 'ga' ? 'General Affairs' : user.role.toUpperCase()}
                  </span>
                </td>
                <td className="px-2 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm">
                  <button
        ...
        <div className="px-4 py-3 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 text-xs lg:text-sm text-gray-600">
            <span>Menampilkan {startIndex + 1}-{Math.min(endIndex, filteredUsers.length)} dari {filteredUsers.length} data</span>
          </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={formData.role}
                  onChange={(e) => setFormData({ ...formData, role: e.target.value as 'admin' | 'head_dept' | 'ga' | 'user' })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                >
                  <option value="user">User</option>
                  <option value="admin">Admin</option>
                  <option value="head_dept">Head Department</option>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={formData.role}
                  onChange={(e) => setFormData({ ...formData, role: e.target.value as 'user' | 'admin' | 'head_dept' | 'ga' })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                >
                  <option value="user">User</option>
                  <option value="admin">Admin</option>
            ...
            <div className="px-6 py-4">
              ...
              <div className="mb-6">
                ...
                <div
                  ...
                  onDrop={handleDrop}
                  onClick={() => document.getElementById('import-file-input-user')?.click()}
                  className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${
                    dragOver ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-gray-50'
                  } ${uploading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                >
                  <input
                    id="import-file-input-user"
                    type="file"
                    accept=".xlsx,.xls"
